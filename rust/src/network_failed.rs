//! P2P networking layer using libp2p (updated for current API)

use libp2p::{
    gossipsub::{self, MessageId, Topic, TopicHash, IdentTopic},
    identity::Keypair,
    mdns,
    noise,
...
impl P2PNode {
    ...
    pub async fn new(
        ...
        // Create mDNS behavior for local peer discovery (no await in current API)
        ...
        loop {
            match self.swarm.select_next_some().await {
                SwarmEvent::Behaviour(event) => {
                    ...
                    match event {
                        libp2p::swarm::derive_prelude!(NetworkBehaviour, gossipsub::Event::Message {
                            propagation_source,
                            message_id: _,
                            message,
                        }) => {
                        ...
                        libp2p::swarm::derive_prelude!(NetworkBehaviour, mdns::Event::Discovered(list)) => {
                            for (peer_id, _addr) in list {
                                self.swarm.behaviour_mut().gossipsub.add_explicit_peer(&peer_id);
                            }
                        }
                        libp2p::swarm::derive_prelude!(NetworkBehaviour, mdns::Event::Expired(list)) => {
                            for (peer_id, _addr) in list {
